...
- 2026-09-23 v0.1.3 发布：REQ-2026-008–011 随 bug-fix 分支合并入 main。发布验收以实际 ZIP 干净解压执行，`file://` 与明文 `http` 局域网 IP（192.168.31.26，实测 `isSecureContext: false`、`crypto.randomUUID` 缺失）双场景：渲染、完整对局（下注→发牌→停牌→结算→下一局）、发牌动画中刷新恢复、导出→改动→导入往返（余额精确恢复），全程零控制台/页面错误。Vitest 64/64、typecheck、format:check、build 全绿。
